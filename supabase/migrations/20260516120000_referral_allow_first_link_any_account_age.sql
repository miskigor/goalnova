-- Allow first-time referral for any account age (30-day rule blocked old emails like ivano / miamiskulin).
-- Abuse protection: unique referred_user_id, referred_by, no self-referral.
...
begin
  ...
  if exists (
    select 1 from public.player_referrals r where r.referred_user_id = me
  ) then
