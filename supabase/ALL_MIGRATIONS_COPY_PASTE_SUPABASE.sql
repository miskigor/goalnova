...
-- Referral rules: composite uniqueness + complete only for recently created accounts

-- One row per (referrer, referred) pair (idempotent with insert on conflict)
create unique index if not exists player_referrals_referrer_referred_uidx
on public.player_referrals (referrer_user_id, referred_user_id);

-- ---------------------------------------------------------------------------
-- Complete referral: only "new" accounts (auth.users.created_at within window)
-- ---------------------------------------------------------------------------
create or replace function public.goalnova_player_complete_referral(p_referral_code text)
returns jsonb
language plpgsql
...
begin
  ...
  -- Only count referrals for accounts created in the last 30 days (new signup path).
