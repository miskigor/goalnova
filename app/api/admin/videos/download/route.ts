...
import { resolveAdminVideoDownload } from "@/lib/supabase/adminVideoDownload.server";
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const resolved = await resolveAdminVideoDownload(service, videoId);
  if (!resolved.ok) {
    const status =
      resolved.reason === "not_found"
        ? 404
        : resolved.reason === "no_url"
          ? 422
          : 500;
    return NextResponse.json(
  ...
  await service.from("admin_audit_log").insert({
    ...
    target_user_id: resolved.ownerUserId,
    action: "download_video",
    details: { video_id: videoId, filename: resolved.filename },
  });
