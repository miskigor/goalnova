...
export function FeedVideoEngagement({
  ...
  return (
    <div
      className={[
        rail ? "relative space-y-1.5" : "space-y-3",
        className,
      ]
        .filter(Boolean)
        .join(" ")}
    >
      ...
      <div
        className={
          rail
            ? "flex w-full touch-manipulation flex-col items-center gap-3"
            : compactUi
      ...
      >
        <button
          ...
          className={[
            ...
            liked
              ? "border-gn-accent/60 bg-gn-accent/20 text-gn-accent shadow-[0_0_20px_-4px_rgba(249,115,22,0.45)]"
              : "border-white/[0.12] bg-white/[0.06] text-white/85 hover:border-gn-accent/40 hover:bg-gn-accent/10 hover:text-gn-accent",
          ].join(" ")}
          aria-pressed={liked}
        >
          <span aria-hidden className={rail ? "text-lg leading-none" : compactUi ? "text-[13px] leading-none" : "text-base leading-none"}>
            {liked ? "♥" : "♡"}
          </span>
          {rail ? (
        ...
        >
          {compactUi || rail ? (
            <CommentGlyph
              className={
                rail ? "size-[18px] shrink-0 text-current opacity-95" : "size-3.5 shrink-0 text-current opacity-90"
              }
            />
          ) : (
            t("comment")
